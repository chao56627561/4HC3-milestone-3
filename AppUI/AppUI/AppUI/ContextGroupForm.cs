...
namespace AppUI
{
    public partial class ContextGroupForm : Form
    {
        ...
        public ContextGroupForm(_tagChatRecord record)
        {
            InitializeComponent();
        ...
        private void ContextGroupForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            this.Close();
            m_From_B.reOrder();
            m_From_B.Show();
        ...
        private void ContextGroupForm_Load(object sender, EventArgs e)
        {
        ...
        private class PanelExtend : Panel
        {
            ...
            public PanelExtend(_tagChatContexts record, string imgPath, int width)
            {
                ...
                int msgWidth = (int)sizeF.Width + 10;
                sizeF = graphics.MeasureString(record.name, new Font("Arial", 9));
                int nameWidth = (int)sizeF.Width + 10;
                graphics.Dispose();

                Label lblName = new Label();
                lblName.Text = record.name;
                lblName.Font = new Font("Arial", 9);
                lblName.Width = nameWidth;
                lblName.Height = 15;
                this.Controls.Add(lblName);
                if (record.tag != -1)
                    lblName.Location = new Point(50, 2);
                else
                    lblName.Location = new Point(picBox.Location.X - 5 - lblName.Width, 2);

                Label lblMsg = new Label();
                lblMsg.Text = record.msg;
                lblMsg.Font = new Font("Arial", 9);
                lblMsg.Width = msgWidth;
                lblMsg.Height = 15;
                lblMsg.BackColor = Color.White;
                this.Controls.Add(lblMsg);
                if (record.tag != -1)
                    lblMsg.Location = new Point(50, 22);
                else
                    lblMsg.Location = new Point(picBox.Location.X - 5 - lblMsg.Width, 22);
            }
        }
    }
}
